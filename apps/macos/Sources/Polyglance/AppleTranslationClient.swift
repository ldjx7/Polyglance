...
final class AppleTranslationClient: TranslationClient, @unchecked Sendable {
    ...
    func translate(_ request: AppTranslationRequest) async throws -> AppTranslationResult {
        ...
        throw NSError(
            ...
            code: 1000,
            userInfo: [NSLocalizedDescriptionKey: "Apple 系统翻译仅支持 macOS 15.0（Sequoia）及更高版本。"]
        )
    }

    #if canImport(Translation)
    @available(macOS 15.0, *)
    private func executeTranslation(_ request: AppTranslationRequest) async throws -> AppTranslationResult {
        ...
        do {
            let translatedText = try await AppleTranslationBridge.shared.translate(
                text: request.text,
                source: sourceLang,
                target: targetLang
            )
            ...
            if "\(error)".contains("notInstalled") || error.localizedDescription.contains("notInstalled") {
                throw NSError(
                    domain: "Polyglance.AppleTranslation",
                    code: 1001,
